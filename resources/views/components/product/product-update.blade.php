...
<script>
    ...
    async function FillUpUpdateForm(id, filePath) {
        ...
        let res = await axios.post("/product_by_id", {
            id: id
        })
        hideLoader();
    ...
    async function update() {
        let productCategoryUpate = document.getElementById('productCategoryUpdate').value;
        ...
        if (productCategoryUpate.length === 0) {
            ...
            formData.append('category_id', productCategoryUpate)
            ...
            showLoader();
            let res = await axios.post("/product_update", formData, config)
            hideLoader();

            if (res.status === 200 && res.data === 1) {
                successToast("Product Update Successfull");
